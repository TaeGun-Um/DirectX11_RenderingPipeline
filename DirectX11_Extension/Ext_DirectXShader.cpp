...
void Ext_DirectXShader::ShaderAutoCompile(std::string_view _Path, std::string_view _EntryPoint)
{
	...
	{
		MsgAssert("Compute 셰이더 타입은 아직 안만듬");
...
void Ext_DirectXShader::ShaderResourceSetting()
{
	...
	for (UINT i = 0; i < Info.BoundResources; i++)
	{
		...
		case D3D_SIT_STRUCTURED:
		{
			...
			//std::shared_ptr<Ext_DirectXStructuredBuffer> Res = Ext_DirectXStructuredBuffer::CreateAndFind(Name, BufferDesc, 0);

			//StructuredBufferSetter Setter;
			//Setter.ParentShader = this;
			//Setter.Name = UpperName;
			//Setter.BindPoint = ResDesc.BindPoint;
			//Setter.Res = Res;

			//BufferSetter.CreateStructuredBufferSetter(Setter);
			break;
		}
