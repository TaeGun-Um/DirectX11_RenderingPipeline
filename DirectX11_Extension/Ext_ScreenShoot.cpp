...
HRESULT Ext_ScreenShoot::SaveDDSTextureToFile
(
...
) noexcept
{
    ...
    HRESULT hr = TextureCapture(pContext, pSource, desc, pStaging);
    if (FAILED(hr))
        return hr;
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT Ext_ScreenShoot::SaveWICTextureToFile
(
    ID3D11DeviceContext* pContext,
    ID3D11Resource* pSource,
...
HRESULT Ext_ScreenShoot::ScreenShoot()
{
    ID3D11Texture2D* buffer = Ext_DirectXDevice::GetMainRenderTarget()->GetTexture(0)->GetTexture2D();

    if (nullptr != buffer)
    {
        Base_Directory Dir;
        Dir.MakePath("../ScreenShot");

        wchar_t PrevPath[255];
        GetCurrentDirectoryW(255, PrevPath);

        std::string Path = Dir.GetPath();
        // 경로 변경

        SetCurrentDirectoryW(Base_String::AnsiToUniCode(Path).data());
        SaveWICTextureToFile(Ext_DirectXDevice::GetContext(), buffer, GUID_ContainerFormatJpeg, L"SCREENSHOT.JPG");
        SetCurrentDirectoryW(PrevPath);
    }

    return S_OK;
}

HRESULT Ext_ScreenShoot::RenderTargetShoot(std::shared_ptr<Ext_DirectXRenderTarget> _CaptureTarget, std::string_view _Path, std::string_view _TextureName)
{
    ...
    if (nullptr != Resource)
    {
        ...
        std::shared_ptr<Ext_DirectXRenderTarget> MergeTarget = Ext_DirectXRenderTarget::CreateRenderTarget(DXGI_FORMAT::DXGI_FORMAT_B8G8R8A8_UNORM, float4((float)TexInfo.Width, (float)TexInfo.Height), float4::ZERONULL);
        ...
        SaveDDSTextureToFile(Ext_DirectXDevice::GetContext(), MergeTarget->GetTexture(0)->GetTexture2D(), Base_String::AnsiToUniCode(_TextureName).data());
