    ...
    std::vector<unsigned int>& _OutIndices)
{
    ...
    const aiMesh* AIMesh = MeshScene->mMeshes[0];
    _OutVertices.clear();
    _OutIndices.clear();
    _OutVertices.reserve(AIMesh->mNumVertices);
    _OutIndices.reserve(AIMesh->mNumFaces * 3);
    ...
    for (unsigned int v = 0; v < AIMesh->mNumVertices; ++v)
    {
        ...
        // (A) POSITION
        const aiVector3D& Ap = AIMesh->mVertices[v];
        VData.POSITION = float4(Ap.x, Ap.y, Ap.z, 1.0f);

        // (B) NORMAL
        if (AIMesh->HasNormals())
        {
            const aiVector3D& An = AIMesh->mNormals[v];
        ...
        {
            const aiVector3D& Au = AIMesh->mTextureCoords[0][v];
