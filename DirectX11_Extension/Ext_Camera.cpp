...
void Ext_Camera::Rendering(float _Deltatime)
{
	...
	for (auto& [RenderPathKey, UnitMap] : MeshComponentUnits)
	{
		...
		// 4. GameEngineDevice::GetContext()->Map(Buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &SettingResources);, memcpy_s(SettingResources.pData, BufferInfo.ByteWidth, _Data, BufferInfo.ByteWidth);, GameEngineDevice::GetContext()->Unmap(Buffer, 0); 호출
}
