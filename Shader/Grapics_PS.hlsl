...
//    Output.PositionTarget = float4(_Input.WorldViewPosition.xyz, 1.0f); // 뷰스페이스 Position
//    Output.NormalTarget = float4(_Input.WorldViewNormal, 1.0f); // 뷰스페이스 Normal
...
float4 Grapics_PS(PSInput _Input) : SV_TARGET
{
    ...
    for (int i = 0; i < LightCount; ++i)
    {
        ...
        // float3 PixelPosition = mul(_Input.WorldViewPosition, (float3x3) LTData.CameraViewInverseMatrix);
        // float3 PixelNormal = normalize(mul(_Input.WorldViewNormal, (float3x3) LTData.CameraViewInverseMatrix));
