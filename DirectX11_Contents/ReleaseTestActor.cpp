...
// 매 프레임 호출
// - AccTime 이 CyclePeriod 를 넘으면 한 사이클 완료: 스폰/디스트로이를 교대로 수행
//     bSpawned == false : 새 배치 스폰 (Cube/Sphere 교대 배치)
//     bSpawned == true  : 기존 배치 전원 Destroy (죽음 예약 → Scene::Release 에서 실제 정리)
// - 결과적으로 CyclePeriod 동안 보임 → CyclePeriod 동안 비어있음 → 반복
